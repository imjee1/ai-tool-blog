...
GitHub Actions (매일 새벽, 크론)
   → scripts/generate_post.py 실행
     ...
     → _posts/에 마크다운 파일 생성
   → 자동 커밋 & 푸시
   → GitHub Pages가 자동으로 빌드해서 블로그에 반영
```
...
2. Source를 **Deploy from a branch** → `main` 브랜치, `/ (root)` 선택 → Save
3. 몇 분 후 `https://내아이디.github.io/ai-tool-blog/` 에서 블로그 확인 가능

### 4. 저장소 이름이 `ai-tool-blog`가 아니라면
`_config.yml` 안의 `baseurl` 주석을 풀고 본인 저장소 이름으로 수정하세요.

### 5. 잘 도는지 수동으로 먼저 테스트
- 저장소 → **Actions** 탭 → `Daily AI tool post` 워크플로우 선택 → **Run workflow** 버튼으로 바로 1회 실행해볼 수 있습니다.
- 문제없이 돌면 `_posts/` 폴더에 오늘 날짜 파일이 생기고 블로그에 반영됩니다.
